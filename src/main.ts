...
interface Stock {
  ...
  conviction_coverage:  number;
  conviction_clarity:   number;
  conviction_unanimity: number;
  conviction_tenure:    number;
...
let emailPrefs: EmailPrefs | null = null;
let sortKey   = "rank";
let sortAsc   = true;
let secFilter    = "All";
let conFilter    = "All";
...
async function refreshPrices() {
  ...
  try {
    ...
    if (isWatchlistPage) { applyFilters(); renderWatchlistRows(); }
    else                  { applyFilters(); renderRows(); }
  } catch {
...
async function toggleWatchlist(ticker: string, starEl: HTMLElement, isLocked: boolean = false) {
  if (isLocked) {
  ...
  if (!proToken && !freeEmail) {
    ...
    if (adding) { watchlist.delete(ticker); starEl.textContent = "☆"; starEl.classList.remove("wl-active"); }
    else        { watchlist.add(ticker);    starEl.textContent = "★"; starEl.classList.add("wl-active"); }
...
function convictionBadge(s: Stock): string {
...
// ── Paywall modal ──────────────────────────────────────────────────────────────
function emailPrefsModal(): string {
  const sectors = stats ? Object.keys(stats.sectors).sort() : [];
...
function bindEmailPrefs() {
  ...
  if (closeBtn) closeBtn.onclick = () => closeModal("ep");

  document.getElementById("ep")?.addEventListener("click", e => {
    if ((e.target as HTMLElement).id === "ep") closeModal("ep");
  });

  const saveBtn = document.getElementById("ep-save") as HTMLButtonElement;
  if (!saveBtn) return;

  saveBtn.onclick = async () => {
    const sector    = (document.getElementById("ep-sector")    as HTMLSelectElement)?.value || "All";
    const consensus = (document.getElementById("ep-consensus") as HTMLSelectElement)?.value || "All";
    const minA      = parseInt((document.getElementById("ep-analysts") as HTMLSelectElement)?.value || "0");
    const maxPe     = parseFloat((document.getElementById("ep-pe")  as HTMLSelectElement)?.value || "0");
    const maxPeg    = parseFloat((document.getElementById("ep-peg") as HTMLSelectElement)?.value || "0");
    const momentum  = (document.getElementById("ep-momentum") as HTMLSelectElement)?.value || "All";
...
function bindGlobals() {
  ...
  if (bEP) bEP.onclick = showEP;
  if (document.getElementById("ep")) bindEmailPrefs();
...
if (subBtn) subBtn.onclick = async () => {
  ...
  if (btnExport) {
    btnExport.onclick = async () => {
      const scopeEl = document.getElementById("export-scope") as HTMLSelectElement;
  ...
  if (btnExportLocked) btnExportLocked.onclick = () => showPW();
