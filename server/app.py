...
def api_changes():
    ...
    # Require the past snapshot to be within the claimed window — otherwise
    # a ticker with only one snapshot from day 1 of collection would compare
    # today vs. months ago and show a spurious "Major upgrade" for what is
    # really just Yahoo's rolling aggregate drifting over time (e.g. a stale
    # analyst's rating expiring out of Yahoo's 12-month window, causing
    # "Underperform" to flip to "Buy" with no actual recent firm action).
    #
    # Tolerance: allow the past snapshot to be up to 50% older than the
    # requested window. So for a 7-day comparison the past snapshot must
    # be between 7 and ~10 days ago; for 30 days, between 30 and 45 days
    # ago; for 90 days, between 90 and 135 days ago. Tickers that haven't
    # accumulated enough history to meet this threshold are silently
    # omitted rather than shown with a misleading date label.
    tolerance       = days * 0.5
    ...
    # The WHERE past.date >= earliest_cutoff clause is the key fix: it
    # rejects past snapshots that are too old to be meaningful for the
    # selected period. Without this, a stock with only one old snapshot
    # always appears in results with its full history-to-date delta,
    # regardless of which period button (7d/30d/90d) the user selected.
    results = con.execute("""
        SELECT
            curr.ticker,
